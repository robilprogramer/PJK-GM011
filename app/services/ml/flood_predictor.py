...
flood_predictor.py — SiagaAI ML Inference Service
===================================================
Model di-load dari Hugging Face Hub secara otomatis saat startup.
File di-cache lokal di folder models/ supaya tidak download ulang
jika tidak ada perubahan (huggingface_hub handles caching via etag).

CARA PAKAI:
  Set environment variable:
    HF_REPO_ID=robil/siagaai-flood-risk-model   (opsional, ada default)
    HF_TOKEN=hf_xxxxxxxxxxxx                     (opsional, untuk private repo)

  Jalankan backend → model auto-download dari HF saat startup.

  Untuk force reload model terbaru tanpa restart:
    flood_predictor.reload_latest()
"""
...
# ─── HF Config (dari env, ada default) ───────────────────────
from app.core.config import settings
_HF_REPO_ID = settings.HF_REPO_ID
_HF_TOKEN   = settings.HF_TOKEN or None  # empty string → None supaya HF treat sebagai public  # None = public repo, tidak perlu token
...
class FloodPredictor:
    """
    Load trained model dari Hugging Face Hub, cache lokal di models/.
    huggingface_hub otomatis skip download jika file sudah ada & hash sama.
    """
    ...
    def __init__(
        ...
        hf_token:    Optional[str] = _HF_TOKEN,
    ):
        self.models_dir = models_dir
    ...
    # ── PERUBAHAN UTAMA: download dari HF sebelum load ────────
    def ensure_loaded(self) -> None:
        if self._loaded:
            return
    ...
    def _download_from_hf(self) -> None:
        """
        Download 4 file model dari HF ke models/.
        Otomatis skip jika file lokal sudah up-to-date (etag check).
        """
        try:
            from huggingface_hub import hf_hub_download
        except ImportError:
            raise ImportError(
                "huggingface_hub tidak terinstall.\n"
        ...
        logger.info("Checking model from Hugging Face...", repo=self.hf_repo_id)

        for filename in files:
            local_path = os.path.join(self.models_dir, filename)
            try:
                # hf_hub_download download ke HF cache global (~/.cache/huggingface)
                # lalu kita copy ke models/ supaya path tetap konsisten
                cached = hf_hub_download(
                    repo_id   = self.hf_repo_id,
                    filename  = filename,
                    token     = self.hf_token,
                    local_dir = self.models_dir,   # langsung simpan ke models/
                )
                logger.info("Model file ready", file=filename, path=cached)
            except Exception as e:
                # Kalau download gagal (offline/rate limit), coba pakai file lokal
                if os.path.exists(local_path):
                    logger.warning(
                        "HF download failed, using cached local file",
                        file=filename, error=str(e)
                    )
                else:
                    raise RuntimeError(
                        f"Gagal download '{filename}' dari HF dan tidak ada cache lokal.\n"
                        f"Repo: {self.hf_repo_id}\nError: {e}"
                    )

    def _load_from_disk(self) -> None:
        """Load model ke memory dari folder models/ (sama persis seperti sebelumnya)."""
        model_path  = os.path.join(self.models_dir, "flood_risk_model.pkl")
        scaler_path = os.path.join(self.models_dir, "feature_scaler.pkl")
        enc_path    = os.path.join(self.models_dir, "label_encoder.pkl")
        meta_path   = os.path.join(self.models_dir, "model_metadata.json")
        ...
        missing = [p for p in [model_path, scaler_path, enc_path, meta_path]
                   if not os.path.exists(p)]
        if missing:
            raise FileNotFoundError(f"Model files tidak ditemukan setelah download: {missing}")
        ...
        logger.info(
            ...
            accuracy = self._metadata.get("accuracy"),
            f1       = self._metadata.get("f1_score_weighted"),
            source   = f"hf:{self.hf_repo_id}",
        )

    def reload_latest(self) -> None:
        """
        Force pull model terbaru dari HF lalu reload ke memory.
        Bisa dipanggil dari endpoint admin tanpa restart server.
        """
        logger.info("Reloading model from Hugging Face...", repo=self.hf_repo_id)
        self._loaded = False
        self._model = self._scaler = self._le = None
        self._metadata = {}
        self.ensure_loaded()
        logger.info("Model reloaded", version=self._metadata.get("model_version"))
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "top5_features"          : self._metadata.get("top5_features", []),
            "is_loaded"              : self._loaded,
            "hf_repo"                : self.hf_repo_id,   # tambahan info sumber model
        }
