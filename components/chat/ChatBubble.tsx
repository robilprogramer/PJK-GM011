...
export function ChatBubble({ message }: Props) {
  ...
  return (
    <div className={cn('flex gap-2.5 animate-slide-up', isUser && 'flex-row-reverse')}>
      {/* Avatar */}
      <div className={cn(
        'flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center text-white text-sm',
      ...
      <div className={cn('flex flex-col gap-1 max-w-[80%]', isUser && 'items-end')}>
        {/* Intent Badge */}
        {intent && !isUser && (
          <span className={cn('inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium', intent.color)}>
        ...
        <div className={cn(
          ...
          {message.requires_escalation && !isUser && (
            <div className="flex items-center gap-1.5 text-red-600 font-semibold mb-2 text-xs">
          ...
          {message.isStreaming ? (
            <div className="flex items-center gap-1 py-1">
              {[0,1,2].map(i => (
                <div key={i} className="w-2 h-2 bg-slate-400 rounded-full typing-dot" />
              ))}
            </div>
          ) : (
        ...
        <div className="flex items-center gap-2">
          ...
          {message.model_used && !isUser && (
            <span className="text-xs text-slate-300 bg-slate-100 px-1.5 py-0.5 rounded">
